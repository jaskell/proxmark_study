...
func update_fbc21_fbc24(semi *SemiState, shiftcnt uint) {
...
func extend_table(table *SemiStateTable, b bit, extcnt uint) *SemiStateTable {
	...
	for cur != nil {
		...
		if flag == 0 {
			...
			semi.value = extval1
			if extcnt > 4 {
				update_fbc21_fbc24(semi, extcnt-4)
			}
		} else if flag == 2 {
			semi.value = extval2
			if extcnt > 4 {
			...
			if extcnt > 4 {
				update_fbc21_fbc24(semi, extcnt-4)
				update_fbc21_fbc24(newsemi, extcnt-4)
			}
		}
		cur = next
	}
...
func init_evnheap(tbl *SemiStateTable) *EvnStateHeap {
	sheap := new(EvnStateHeap) //, tbl.Len())
	for elem := tbl.Front(); elem != nil; elem = elem.Next() {
		sheap.Push(elem.Value.(*SemiState))
	}
	return sheap
}

func init_oddheap(tbl *SemiStateTable) *OddStateHeap {
	sheap := new(OddStateHeap) //, tbl.Len())
	for elem := tbl.Front(); elem != nil; elem = elem.Next() {
		sheap.Push(elem.Value.(*SemiState))
	}
	return sheap
